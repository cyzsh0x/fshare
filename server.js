...
async function getNextSessionNumber() {
  ...
  // Update the counter to the highest number we've used
  if (nextNumber > currentValue) {
...
async function saveProgress(sessionId, progress) {
  ...
  // Check if session is completing or failing
  const isCompleting = ['completed', 'failed', 'terminated'].includes(progress.status) && 
                       sessions[sessionId].status === 'in_progress';
  ...
  // If session is completing, we might need to adjust the counter
  if (isCompleting) {
    await adjustSessionCounter();
  }
  
  await broadcastActiveSessions();
}

async function adjustSessionCounter() {
  const sessions = await readSessions();
  const activeSessions = Object.values(sessions).filter(s => ['started', 'in_progress'].includes(s.status));
  
  if (activeSessions.length === 0) {
    // No active sessions, reset counter to 0
    await counterRef.set(0);
  } else {
    // Set counter to the highest session number in use
    const maxSessionNumber = Math.max(...activeSessions.map(s => s.sessionNumber));
    const currentCounter = (await counterRef.once('value')).val() || 0;
    
    if (maxSessionNumber > currentCounter) {
      await counterRef.set(maxSessionNumber);
    }
  }
}
...
async function shareInBackground(sessionId, cookie, url, amount, interval) {
  try {
    ...
    const maxRetries = 3;
    const batchSize = interval < 1 ? Math.min(10, amount) : 1;
    const batchInterval = interval < 1 ? batchSize * interval * 1000 : interval * 1000;
    ...
    while (sharesDone < amount) {
      const batchStartTime = Date.now();
      const currentBatchSize = Math.min(batchSize, amount - sharesDone);
      ...
      // Update progress at least every second or when batch completes
      if (Date.now() - lastUpdateTime >= 1000 || sharesDone === amount) {
        await saveProgress(sessionId, {
          completedShares: successCount,
          failedShares: failedCount
        });
